...
from magpie_mongo.config import WORD2VEC_MODELPATH, EMBEDDING_SIZE
from magpie_mongo.utils import get_documents, save_to_disk, get_documents_from_mongo
...
def fit_scaler_mongo(mongo_collection, train_ids, word2vec_model=WORD2VEC_MODELPATH, batch_size=1024,
               persist_to_path=None):
    """ Get all the word2vec vectors in a 2D matrix and fit the scaler on it.
     This scaler can be used afterwards for normalizing feature matrices. """
    if type(word2vec_model) == str:
    ...
    doc_generator = get_documents_from_mongo(train_ids, mongo_collection)
    scaler = StandardScaler(copy=False)
