...
from magpie_mongo.base.word2vec import train_word2vec, fit_scaler, fit_scaler_mongo
from magpie_mongo.config import NN_ARCHITECTURE, BATCH_SIZE, EMBEDDING_SIZE, NB_EPOCHS
from magpie_mongo.nn.input_data import get_data_for_model, get_data_for_model_mongo
from magpie_mongo.nn.models import get_nn_model
...
class MagpieModel(object):
    ...
    def batch_train_mongo(self, mongo_collection, train_ids, vocabulary, test_ids=None, callbacks=None,
                    nn_model=NN_ARCHITECTURE, batch_size=BATCH_SIZE,
                    nb_epochs=NB_EPOCHS, verbose=1):
        """
        Train the model on given data
        ...
        train_generator, test_data = get_data_for_model_mongo(
            mongo_collection,
            train_ids,
            vocabulary,
            test_ids=test_ids,
    ...
    def fit_scaler_mongo(self, mongo_collection, train_ids):
        """
        Fit a scaler on given data. Word vectors must be trained already.
        :param train_dir: directory with '.txt' files
        ...
        self.scaler = fit_scaler_mongo(mongo_collection, train_ids, word2vec_model=self.word2vec_model)
